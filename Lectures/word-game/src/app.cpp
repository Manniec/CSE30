...
int main(int argc, char** argv){
    ...
    server.route("/checkWord", [=](const request& req, response& res){ //endpoint 
        if (req.url_params.hasKey("word")){
            std::string word = req.url_params.get("word");

            ucm::json answer = checkWord(word); //mylib.h
    ...
    server.route("/list", [](const request& req, response& res){ //list endpoint
